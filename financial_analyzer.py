...
DEFAULT_CONFIG = {
    ...
    "change_threshold": 0.05  # 5% change threshold for notifications
}
...
if args.config_file:
    try:
        with open(args.config_file, 'r') as config_file:
            ...
            EMAIL_CONFIG = config.get("email_config", DEFAULT_CONFIG["email_config"])
            CHANGE_THRESHOLD = config.get("change_threshold", DEFAULT_CONFIG["change_threshold"])
...
def scrape_stockanalysis(stock_symbol):
    """
    Scrapes financial data from stockanalysis.com for the specified stock symbol.
    ...
    try:
        ...
        try:
            tables = soup.find_all("table")
            for table in tables:
                if "Revenue" in table.text or "Net Income" in table.text:
                    rows = table.find_all("tr")
                    for row in rows:
                        cols = row.find_all("td")
                        if len(cols) >= 2:
                            ...
                            financial_data[key] = float(value) if value.replace(".", "").isdigit() else value
        except Exception as e:
            logging.warning(f"Error extracting financial data from stockanalysis.com for {stock_symbol}: {e}")

        return {"financial_data": financial_data, "news": []}
    except requests.exceptions.HTTPError as e:
        logging.error(f"StockAnalysis: HTTP error for {stock_symbol}: {e}")
        return None
    except Exception as e:
        logging.error(f"Error scraping stockanalysis.com for {stock_symbol}: {e}")
        return None
...
def aggregate_data(stock_symbol, exchange_code=None):
    ...
    sources = [
        ...
        scrape_stockanalysis,  # Added stockanalysis.com as a source
    ]
...
def send_email_notification(subject, body):
    ...
    """
    if not EMAIL_CONFIG["smtp_server"] or not EMAIL_CONFIG["sender_email"] or not EMAIL_CONFIG["receiver_email"]:
...
def detect_significant_changes(old_data, new_data):
    """
    Detects significant changes in financial data or stock prices.

    Args:
        old_data (dict): The previous financial data.
        new_data (dict): The updated financial data.

    Returns:
        bool: True if significant changes are detected, False otherwise.
    """
    try:
        for key in new_data:
            if key in old_data:
        ...
        logging.error(f"Error detecting significant changes: {e}")
        return False

def fetch_and_notify(stock_symbol, exchange_code=None):
    """
    Fetches the latest financial data and sends an email notification if significant changes are detected.
    ...
    if new_data:
        if detect_significant_changes(latest_data, new_data):
            logging.info("Significant changes detected. Sending email notification...")
            subject = f"Significant Changes Detected for {stock_symbol}"
            body = f"New financial data:\n{json.dumps(new_data, indent=2)}"
...
def cleanup_selenium_driver(driver):
    """
...
def scrape_financial_data_selenium(self):
    """
    Scrapes financial data using Selenium with improved error handling and resource cleanup.
...
def validate_financial_data(data):
    """
    Validates the financial data to ensure it meets the required format.

    Args:
        data (pd.DataFrame): The financial data to validate.

    Returns:
        bool: True if the data is valid, False otherwise.
    """
    if data is None or data.empty:
        logging.error("Financial data is empty or None.")
        return False

    required_columns = ["Revenue", "Net Income", "Total Assets", "Total Liabilities"]
...
def load_financial_data(self, file):
    """
    Loads financial data from an uploaded file with enhanced validation.

    Args:
        file (UploadedFile): The uploaded file.
    """
    # ...existing code...
    if self.financial_data is not None and not validate_financial_data(self.financial_data):
