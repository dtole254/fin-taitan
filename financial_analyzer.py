...
class FinancialAnalyzer:
    ...
    def calculate_ratios(self):
        ...
        try:
            ...
            try:
                # Basic Ratios
                ratios['gross_profit_margin'] = ((data[revenue_col] - data[cogs_col]) / data[revenue_col]).mean() * 100 if revenue_col and cogs_col else None
                ratios['net_profit_margin'] = (data[net_income_col] / data[revenue_col]).mean() * 100 if revenue_col and net_income_col else None
                ratios['return_on_assets'] = (data[net_income_col] / data[total_assets_col]).mean() * 100 if net_income_col and total_assets_col else None
                ratios['return_on_equity'] = (data[net_income_col] / data[total_equity_col]).mean() * 100 if net_income_col and total_equity_col else None
                ...
                ratios['equity_multiplier'] = (data[total_assets_col] / data[total_equity_col]).mean() if total_assets_col and total_equity_col else None
    ...
    def start_tracking(self):
        ...
        thread.daemon = True  # Allow the main program to exit
        thread.start()
    ...
    def display_financial_data(self):
        ...
        if st.button("Start Tracking"):
            self.start_tracking()
            st.success(f"Fetching initial data for {self.company_name} ({self.ticker}) on NASDAQ...")
